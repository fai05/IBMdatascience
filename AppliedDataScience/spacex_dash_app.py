...
def scatter(input1, input2):
    ...
    new_df_filtered = new_df[(new_df["Payload Mass (kg)"] >= input2[0]) & 
                             (new_df["Payload Mass (kg)"] <= input2[1])]
    
    fig2 = px.scatter(new_df_filtered, y="class", x="Payload Mass (kg)", 
                      color="Booster Version Category", 
                      title=f"Correlation between Payload and Success for {'All Sites' if input1 == 'All Sites' else input1}")
    return fig2
